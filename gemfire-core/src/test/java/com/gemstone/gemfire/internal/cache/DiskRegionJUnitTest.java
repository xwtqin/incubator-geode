...
public class DiskRegionJUnitTest extends DiskRegionTestingBase
{
  ...
  public void testFaultingInRemovalFromAsyncBuffer()
  {
    ...
    long startTime = System.currentTimeMillis();
    Threads.join(thread1, 20 * 1000);
    Threads.join(thread2, 20 * 1000);
    Threads.join(thread3, 20 * 1000);
    Threads.join(thread4, 20 * 1000);
    Threads.join(thread5, 20 * 1000);
...
//          }
    ...
    long startTime = System.currentTimeMillis();
    finished = true;
    Threads.join(thread1, 5 * 60 * 1000);
    Threads.join(thread2, 5 * 60 * 1000);
    Threads.join(thread3, 5 * 60 * 1000);
    Threads.join(thread4, 5 * 60 * 1000);
    Threads.join(thread5, 5 * 60 * 1000);
    long interval = System.currentTimeMillis() - startTime;
    if (interval > 100000) {
      fail(" Test took too long in going to join, it should have exited before 100000 ms");
    }
  ...
  private void dskAccessExpHelperMethod(final Region region, final boolean synchMode)
  {
    ...
    testThread.start();
   // region.clear();
    Threads.join(testThread, 40 * 1000);
    assertFalse(failureCause, testFailed);
